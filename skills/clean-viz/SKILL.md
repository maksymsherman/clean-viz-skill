...
- **No duplicate colors for distinguishable categories** — every category that the reader needs to tell apart must receive a visually distinct color. When using grayscale for context categories, assign different grays (e.g., `#555555`, `#888888`, `#cccccc`) rather than the same gray for all. This is critical when the total number of categories is small (roughly 8 or fewer) — reusing a color effectively merges categories in the reader's eye. Color reuse is acceptable only when there are too many categories to assign unique values (roughly >8), and even then, combine reused color with another differentiator (marker shape, line style, spatial separation)
